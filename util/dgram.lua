...
function CodeBlock(block)
  ...
  local fname = pandoc.sha1(block.text) .. "." .. filetype
  local infile = paths.filename(PANDOC_STATE.output_file)
  local basename, extin = paths.split_extension(infile)

  if save_diagrams then
    if PANDOC_STATE.output_file ~= nil then
      local output_dir = paths.directory(PANDOC_STATE.output_file)
      image_path = paths.join({"images", "posts", basename .. "-" .. fname})
      local image_store_path = paths.join({output_dir, "..", image_path})
      local f = io.open(image_store_path, "w")
      f:write(img)
      f:close()
      pandoc.mediabag.insert(image_path, mimetype, img)
    end
  else
      image_path = fname
      pandoc.mediabag.insert(image_path, mimetype, img)
  ...
  local img_obj = pandoc.Image(caption, "/" .. image_path, title, img_attr)
